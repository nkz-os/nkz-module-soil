...
class LabAnalysisProvider:
    ...
    async def fetch(self, geometry: dict, properties: list[SoilProperty], depths: list[DepthInterval]) -> SoilDataResult:
        ...
        if not points:
            return SoilDataResult(provider=self.name, horizons=[], uncertainty=0.02, geometry=geometry)
        ...
        for point in points:
            ...
            for h in raw_horizons:
                h_obj = Horizon(
                    ...
                    sand=h.get("sand"), silt=h.get("silt"), clay=h.get("clay"),
                    organic_carbon=h.get("organicCarbon"), bulk_density=h.get("bulkDensity"),
                    ph=h.get("ph"), cec=h.get("cec"), coarse_fragments=h.get("coarseFragments"),
                    penetration_resistance=h.get("penetrationResistance"),
                )
                all_horizons.append(h_obj)

        return SoilDataResult(provider=self.name, horizons=all_horizons, uncertainty=0.02, geometry=geometry)
