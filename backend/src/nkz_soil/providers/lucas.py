...
class LucasPointsProvider:
    ...
    async def fetch(self, geometry: dict, properties: list[SoilProperty], depths: list[DepthInterval]) -> SoilDataResult:
        # Used for cross-validation, not direct ingestion into AgriSoil
        return SoilDataResult(provider=self.name, horizons=[], uncertainty=0.10, geometry=geometry)
