...
async def create_sampling_point(body: SamplingPointInput, tenant_id: str = Depends(get_tenant_id)):
    if body.sand is not None and body.silt is not None and body.clay is not None:
        ...
        if total < 97 or total > 103:
            raise HTTPException(status_code=422, detail=f"sand+silt+clay must be ~100%, got {total}")
    if body.ph is not None and (body.ph < 0 or body.ph > 14):
        raise HTTPException(status_code=422, detail="pH must be 0-14")
    if body.bulk_density is not None and (body.bulk_density < 0.1 or body.bulk_density > 2.65):
        raise HTTPException(status_code=422, detail="bulkDensity must be 0.1-2.65 g/cm3")
    ...
    entity = {
        ...
        "laboratoryReference": {"type": "Property", "value": body.laboratory_reference},
        "horizons": {
            "type": "Property",
            "value": [{
                "depthFrom": body.depth_from, "depthTo": body.depth_to,
                "sand": body.sand, "silt": body.silt, "clay": body.clay,
                "organicCarbon": body.organic_carbon, "bulkDensity": body.bulk_density,
                "ph": body.ph, "cec": body.cec, "coarseFragments": body.coarse_fragments,
...
async def create_survey(body: SurveyInput, tenant_id: str = Depends(get_tenant_id)):
    if body.survey_type not in ("lab", "em", "nir", "auger"):
        raise HTTPException(status_code=422, detail="surveyType must be lab, em, nir, or auger")
    ...
    entity = {
        ...
        "refAgriParcel": {"type": "Relationship", "object": f"urn:ngsi-ld:AgriParcel:{body.parcel_id}"} if body.parcel_id else None,
...
async def force_ingest(parcel_id: str, tenant_id: str = Depends(get_tenant_id)):
