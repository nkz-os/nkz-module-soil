...
async def prometheus_metrics():
    ...
    for summary in metrics.get_all_summaries():
        ...
        lines.append(f'# HELP soil_provider_latency_ms Provider fetch latency in milliseconds')
        lines.append(f'# TYPE soil_provider_latency_ms gauge')
        ...
        lines.append(f'# HELP soil_provider_error_rate Provider error rate')
        lines.append(f'# TYPE soil_provider_error_rate gauge')
        lines.append(f'soil_provider_error_rate{{provider="{name}"}} {summary["error_rate"]:.4f}')
        lines.append(f'# HELP soil_provider_cache_hit_rate Provider cache hit rate')
        lines.append(f'# TYPE soil_provider_cache_hit_rate gauge')
        lines.append(f'soil_provider_cache_hit_rate{{provider="{name}"}} {summary["cache"]["hit_rate"]:.4f}')
        lines.append(f'# HELP soil_provider_fetches_total Total provider fetches')
        lines.append(f'# TYPE soil_provider_fetches_total counter')
        lines.append(f'soil_provider_fetches_total{{provider="{name}"}} {summary["total_fetches"]}')
        lines.append(f'# HELP soil_provider_errors_total Total provider errors')
        lines.append(f'# TYPE soil_provider_errors_total counter')
